$(document).ready(function() {
	...
	$('#btnAddNewExam').click(function() {
		...
		$.ajax({
				...
				type:'POST',
				url:"http://localhost:8080/api/admin/exam/save",
		...
		if(confirm("Bạn muốn xóa bài thi thử này?"))
			{
				...
				$.ajax({
					type:'POST',
					url:"http://localhost:8080/api/admin/exam/delete/"+idBaiThiThu,
	...
	function loadAllBaiThiThu(){
		...
		$.ajax({
			dataType : 'json',
			type:'GET',
			url:"http://localhost:8080/api/admin/exam/loadExam",
			...
			success: function(data){
				...
				for(var i = 0; i <jsonArr.length ;i++) {
		            ...
		            +'class="yellow editBaiThiThu"><button class="btn btn-warning"><i class="fas fa-edit"></i></button></a> '
		     
		            +' <a id="delete.'+jsonArr[i].baithithuid+' "'
		            
		            +'class="red deleteBaiThiThu" ><button class="delete btn btn-danger"><i class="fas fa-trash"></i></button></a> </td>'
