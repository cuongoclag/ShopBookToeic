$(document).ready(function(){
	...
	function ajaxGet(page){
		...
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/api/admin/bai-doc/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
...
//					console.log(baiDoc);
	...
	$(document).on('click', '.btnCapNhatBaiDoc', function(event) {
		...
		var href = "http://localhost:8080/api/admin/bai-doc/"+baiDocId;
    ...
    $(document).on("click",".btnXoaBaiDoc", function() {		
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/api/admin/bai-doc/delete/" + baiDocId,
    ...
    function resetForm(){
 	...
 	function updateFunction(formData, id) {
		...
		$.ajax({
			...
			type : "POST",
			contentType : "application/json",
			url : "http://localhost:8080/api/admin/bai-doc/update/" + id,
			enctype : 'multipart/form-data',
			data : formData,
			// prevent jQuery from automatically transforming the data into a
			// query string
