...
public class FullTestController {
	...
	public String showResultUser(Model model,@PathVariable("correctListening") int correctListening,
		...
		ketquabaitest.setTest(baithithuService.getTest(examId).get(0));
		ketquabaitest.setCorrectListen(correctListening);
		ketquabaitest.setCorrectReading(correctReading);
		ketquabaitest.setNumberCorrectAnswers(correctListening+correctReading);
		ketquabaitest.setNumberWrongAnswers(100-correctListening-correctReading);
